...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, self_attentiopn_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float):
        super().__init__()
        self.self_attention_block = self_attention_block
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        x = self.residua;_connections[0](x, lambda x: self.self_attention_block(x, x, x, tgt_mask))
...
class Transformer(nn.Module):
    ...
    def __init__(self, encode: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings, src_pos: PositionalEncoding, tgt_pos: PositionalEncoding, projection_layer: ProjectionLayer) -> None:
        super().__init__()
    ...
    # We will now define 3 methods, one to encode, one to decode and one to project. We will apply them in succession.
    # Why don't we write one single forward method? We will see later that during inferencing we can use the output of the
    # encoder. We won't have to build it every time. We also keep the outputs of the three methods separate as it helps
    # in visualizing attention better!
...
# Note that we use terms related to Machine Translation here because that is the task we are aiming to do. But, this 
# same transformer model can be used for any task (or all tasks). We use the same hyperparameter values as the paper
